...
public class DSP {
    ...
    private static void getmax(double[] are,double[] aim){

        double max = 0.0f;
        int maxindex = 0;
        for (int i = 0; i < fftlen/2; ++i) {
            if(max < are[i] * are[i] + aim[i] * aim[i])
            {
                max = are[i] * are[i] + aim[i] * aim[i];
                maxindex = i;
            }

        }

        double fre = (44100.0f/fftlen)*maxindex;
...
//AMDF
    ...
    public static double freq(double[] buf, int sr) {
        ...
        Log.d("the freq:",Integer.toString(sr) + " "  +Double.toString(interp) + "  " + Integer.toString(j));
        return (double)sr / (j + 0);
    }
