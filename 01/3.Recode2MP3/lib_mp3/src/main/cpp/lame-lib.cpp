...
Java_com_tb_libmp3_LameEncode_init(JNIEnv *env, jclass jclass1, jint sampleRate,
                                     jint channelCount,
...
Java_com_tb_libmp3_LameEncode_encoder(JNIEnv *env, jclass jclass1, jshortArray pcm_buffer,
...
//关闭MP3编码buffer
extern "C" JNIEXPORT jint JNICALL
Java_com_tb_libmp3_LameEncode_flush(JNIEnv *env, jclass jclass1, jbyteArray mp3_buffer) {
    //获取MP3数组长度
    const jsize mp3_buff_len = env->GetArrayLength(mp3_buffer);
    //获取buffer指针
    jbyte *mp3_buf = env->GetByteArrayElements(mp3_buffer, JNI_FALSE);
...
Java_com_tb_libmp3_LameEncode_close(JNIEnv *env, jclass type) {
    lame_close(gfp);
}